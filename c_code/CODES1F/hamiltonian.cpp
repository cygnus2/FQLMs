...
bool myfunc(matval a, matval b){ return (a.rowpos < b.rowpos); }
...
void constH(std::vector<double> &evals, std::vector<std::vector<double>> &evecs){
   ...
   extern int calc_sign(std::vector<bool>&, int, int, int, int);
   extern void printbasis();
   int i,j,k,p,q;
   int p1,p2,p3,p4;
   int sign;
   std::vector<bool> newstate(2*VOL);
   std::vector<bool> oldstate(2*VOL);
   std::vector<matval> rowscan(VOL+1);  
   ...
   for(i=0;i<NH;i++){
     ...
     */
     ...
     rowscan[stateq].rowpos=i; rowscan[stateq].rowsgn=1; stateq++;
     // check
     if(stateq > (VOL+1)){ printf("Error in bound.\n"); }
     // sort the matrix columns
     std::sort (rowscan.begin(), rowscan.end(), myfunc);
     ...
     rows.push_back(curr_index);
     for(k=0;k<stateq;k++){
       cols.push_back(rowscan[k].rowpos+1); 
       if(rowscan[k].rowpos==i) val.push_back(lam*n_Flip);
       else val.push_back(rowscan[k].rowsgn);
     }
 ...
 /* the plaquette pointers are arranged as 
                p3
             o-------o
             |       |
        p4   |   p   |  p2
...
int calc_sign(std::vector<bool> &old, int p1, int p2, int p3, int p4){
   ...
   if(old[p1] == false){ // c-dag_p1 c-dag_p2 c_p3 c_p4
      ...
      if(count%2 == 1) sign = -sign;   // flip sign for odd no of permutations
      old[p4] = !old[p4];              // flip the occupation no
   ...
   else if(old[p1] == true){ // c-dag_p4 c-dag_p3 c_p2 c_p1
      if((old[p2] != true)&&(old[p3] != false)&&(old[p4] != false)) printf("Error in p2, p3, p4\n"); 
      count = count_if(old.begin()+p1+1, old.end(), IsTrue); 
      if(count%2 == 1) sign = -sign;
      old[p1] = !old[p1];
      count = count_if(old.begin()+p2+1, old.end(), IsTrue);
